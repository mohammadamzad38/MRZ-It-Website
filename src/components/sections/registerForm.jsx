...
import RegiUser from "../../../public/icons/regiUser";

const registerForm = ({ color, inputColor }) => {
                        ...
                        <div className='lg:w-[40%] w-full lg:pb-11'>
                            <SlideUp>
                                <div className={`w-[75px] h-[75px] rounded-lg flex justify-center items-center drop-shadow-3xl bg-white mb-3 text-primary`}>
                                    <RegiUser width={50} height={40} fill={"currentColor"} className={`text-primary`}/>
                                </div>
                                <Title size={"5xl"} className={`md:leading-[140%] ${color}`}>Create an account</Title>
                            ...
                            <SlideUp>
                                <form >
                                    <div className='flex sm:flex-row flex-col items-center gap-4 mb-7.5'>
                                        ...
                                        <div className='w-full'>
                                            <label className={`block mb-1 ${color}`} htmlFor={"email1"}>Email*</label>
                                            <Input id={"email1"} type={"email"} className={`w-full border-2 bg-primary ${inputColor}`} />
                                        </div>
                                    </div>
                                    ...
                                    <div className='flex sm:flex-row flex-col items-center gap-4 mb-7.5'>
                                        ...
                                        <div className='w-full'>
                                            <label className={`block mb-1 ${color}`} htmlFor={"confPass"}>Confirm Password*</label>
                                            <Input id={'confPass'} type={"password"} className={`w-full border-2 bg-primary ${inputColor}`} />
                                        </div>
                                    </div>

                                    <div className='flex justify-end w-full'>
                                        <Button variant={"outline"} className="border-white border text-white hover:text-primary-foreground hover:bg-white w-full">Register</Button>
                                    </div>

                                    <div>
								        <p className={`create-account mt-[1.5rem] text-center ${color}`}>
                                            <span>Already have an account?  </span>
                                            <Link className={`underline ${color}`} href={'/login'}>SingIn</Link>
                                        </p>
                                    </div>
                                </form>
                            </SlideUp>
