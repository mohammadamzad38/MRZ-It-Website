              ...
              {data.map(({ id, path, lable, dropDown, megaMenu }) => {
                return (
                  <li key={id}>
                    <Link
                      onClick={() =>
                        setMainDropDownActive(
                          mainDropDownActive === id ? null : id
                        )
                      }
                      href={path}
                    ...
                    >
                      {lable}
                      {dropDown?.length > 0 || megaMenu?.length ? (
                        <span>
                          <IoIosArrowDown />
                        </span>
                      ) : (
                      ...
                      >
                        {
                          /* start dropdonw */
                          dropDown.map(
                            ({ id, lable, path, tag, subDropdonwn }) => {
                              return (
                                <li
                                  key={id}
                                  onClick={() =>
                                    setSubDropDownActive(
                                      subDropDownActive === id ? null : id
                                    )
                                  }
                                  className=" relative px-7 group/subDropdwon"
                                >
                                  <Link
                                    href={path}
                                  ...
                                  {subDropdonwn && (
                                    <ul
                                      className={`transition-all duration-500 ${
                                        subDropDownActive === id
                                          ? "max-h-[300px] overflow-auto opacity-100 pt-3 no-scrollbar"
                                      ...
                                      {subDropdonwn?.map(
                                        ({ id, lable, path, tag }) => {
                                          return (
                                            <li
                                              key={id}
                                              className="pl-7.5 pr-12 "
                                            >
                                              <Link
                                                href={path}
                                                className="whitespace-nowrap font-semibold text-secondary-foreground dark:text-muted-foreground text-base leading-[100%] py-[11px] px-2 block relative z-[1] after:z-[-1] after:absolute after:left-0 after:top-0 after:w-0 after:transition-all after:duration-300 hover:after:w-full after:h-full after:bg-secondary"
                                              >
                                                {lable}
                                              </Link>
                                            </li>
...

                    {
                      ...
                      megaMenu?.length > 0 && (
                        <ul
                          className={`transition-all duration-500 ${
                            mainDropDownActive === id
                              ? "max-h-[2200px] overflow-auto pt-3 no-scrollbar"
                              : "max-h-0 overflow-hidden"
                          }`}
                        >
                          {megaMenu?.map(({ id, lable, path, src }) => {
                            return (
                              <li
                                key={id}
                                className="group/homeImge bg-background rounded-xl mb-6 max-w-[230px] mx-auto"
                              >
                                ...
                                <h5 className="pt-4 pb-3">
                                  <Link
                                    href={path}
                                    className="font-semibold relative text-muted-foreground text-center block "
                                  >
                                    {lable}{" "}
                                  </Link>
                                </h5>
