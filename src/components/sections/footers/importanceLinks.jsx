                ...
                importanceLinks.map(({ id, links, title }) => {
                    return (
                        <div key={id} className='mt-7.5 lg:mt-0'>
                            ...
                            <ul>
                                {
                                    links.map(({ id, label, path }) => <li key={id} className='mt-2.5'><Link href={path} className={cn(`font-semibold hover:text-primary-foreground transition-all duration-500`, color, linkHoverColor)}>{label}</Link></li>)
                                }
                            </ul>
                        </div>
                    )
