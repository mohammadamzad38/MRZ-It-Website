...
const ProjectsTab = () => {
  ...
  const filterProjects = projectsData.filter(({ categories }) =>
    categories.some((item) => (tab === "All" ? projectsData : item === tab))
  );
      ...
      <div className="max-w-[1350px] mx-auto px-[15px]">
        <Tabs onValueChange={onTabChange} defaultValue="All">
            ...
            <div className="flex flex-wrap md:gap-x-10 gap-x-4 gap-y-2 ">
              {category.map((item, index) => (
                <TabsTrigger
                  value={item}
                  key={index}
                  className="font-bold cursor-pointer text-center text-lg text-foreground data-[state=active]:text-muted-foreground data-[state=active]:shadow-none px-0 py-0"
                >
                  {item}
                </TabsTrigger>
              ))}
          ...
          <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-7.5 mt-10 sm:mt-0">
            {filterProjects.map(({ id, info, src, title, link }) => (
              <TeamCard
                key={id}
                position={info}
                name={title}
